...
def _check_url_path(target_url: list[str]) -> str | None:
    try:
        ...
        print(f"{target_url} URLパスチェック時にエラー発生 | _check_url_path ： {e}")
        return None


# 1. はじめに、モジュールの主要な処理を関数にまとめる
def check_sys_args() -> str | None:
    try:
        if len(sys.argv) != 2:
        ...
        target_site: str | None = _check_url_path(except_filename_args)
        return target_site

    except Exception as e:
        print(f"コマンドライン入力時にエラーが発生しました | check_sys_args ： {e}")
        return None
